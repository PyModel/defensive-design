...
|---|---|---|---|
| Function, type, or module interface | Preconditions, which values can exist, state transitions, ownership of mutable state and resources, error idiom | Validated types at the boundary, exhaustive case handling, explicit checks that survive release builds, structured cleanup; see [code-level design](code-level-design.md) | That a static type or cast validated runtime data; that an assertion guards production input |
